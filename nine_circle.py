...
def distance_fun(lens, est):
    ...
    for i in range(len(est)):
        ...
        for a in range(len(index)):
            ...
            centreX_value[k] = get_neighbour_value(index,k,z_est)
            centreY_value[k] = get_neighbour_value(index,k,y_est)
            k = k+1
        ...
        for j in range(9):
            ...
            #print(n_est_norm)
            count2 = 0 
            centreX1 = centreX_value[j]
            centreY1 = centreY_value[j]
            ...
            '''
            manhatten_distance[i][j] = 1*(abs(y_lense_norm[i] - value[j])) + 1*(abs(z_lense_norm[i]-value2[j])) + 1*(abs(n_lense_norm[i]- value3[j])) + 0.3*(abs(range_norm[0] - range_est_norm[0]) + abs(range_norm[1]-range_est_norm[1]) + abs(range_norm[2]-range_est_norm[2]) + abs(range_norm[3] - range_est_norm[3]) + abs(range_norm[4] - range_est_norm[4]))                                      
